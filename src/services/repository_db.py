...
class RepositoryDB(BaseRepository, Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def update(
        ...
        obj_update_data = jsonable_encoder(db_obj)
        updated_db_object = self._model(id=db_obj.id, created_at=db_obj.created_at, **obj_update_data)
        db.add(updated_db_object)
        await db.commit()
        await db.refresh(updated_db_object)
